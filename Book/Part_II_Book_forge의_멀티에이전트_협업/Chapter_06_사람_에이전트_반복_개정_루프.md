...
`book-forge plan <slug> --revise`로 승인 루프를 직접 돌려보되, 일부러 같은 피드백을 3번 연속 입력해보라 — `LoopDetectionConfig(consecutive_repeat_threshold=3)`가 실제로 개입해 차단하는 것을 직접 관찰할 수 있다(§6.2). **여러분이 사람 피드백을 반영해 재생성하는 루프를 만든다면**: "라운드 수 상한"과 "같은 내용 반복 탐지"라는 두 안전장치를 반드시 함께 두라 — 하나만으로는 서로 다른 위험(§6.2의 표)을 놓친다.
