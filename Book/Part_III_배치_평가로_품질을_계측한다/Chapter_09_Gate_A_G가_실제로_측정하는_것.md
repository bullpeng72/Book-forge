...
0장(§0.6)에서 실행한 `book-forge gate`의 실제 출력을 다시 열어, §9.1의 Tracker·Config·Gate 표와 나란히 놓고 대조해보자. 화면에 찍힌 각 줄(`A (목표 달성): 0.663` 등)이 어떤 Tracker의 계산값이고, 어떤 Config가 그 판정 기준을 조정했는지 하나씩 짚어낼 수 있다면 이 챕터를 제대로 소화한 것이다. N/A로 나온 Gate가 있다면 왜 N/A인지(§9.3)도 함께 설명해보라. "그 Gate가 측정할 신호 자체가 이 태스크에 없었다"는 답이 나와야 한다.
