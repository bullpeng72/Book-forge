# Chapter 16. 품질 기준을 프로젝트마다 다르게 — 설정 가능한 Harness
...
## 16.1 발견 — 이미 있던 파라미터가 아무도 안 쓰고 있었다
...
## 16.2 배선 — CLI 플래그로 LLM Judge를 옵트인시킨다
...
## 16.3 Gate 가중치 — 왜 CLI 플래그가 아니라 `.env`인가

10장이 다룬 방법론(실패 모드에서 Gate를 도출하고, 회복 난이도·발생 빈도·사용자 가시성으로 가중치를 정하는 것)이 "무엇을, 왜"를 답했다면, 이 절은 그 결정을 실제 코드로 어떻게 조정 가능하게 만들었는지 — "어떻게"를 답한다.

> 📄 **파일**: `src/book_forge/eval/monitor.py`
...
Gate 가중치는 매번 명령을 입력할 때마다 타이핑하는 값이 아니라 "이 환경에서는 항상 이 기준으로 판정한다"는 **저자별 고정 설정**에 가깝다. 그렇게 판단해 `.env`로 노출했다 — 1장에서 다룬 `LLM_PROVIDER` 선택과 정확히 같은 성격이다. `build_book_monitor()`가 이 값을 `**_gate_weight_overrides()`로 `PerformanceMonitor`에 그대로 펼쳐 넘긴다.

> 📄 **파일**: `src/book_forge/eval/monitor.py` (`build_book_monitor()`)
...
위 생성자에서 아직 안 짚은 나머지 인자도 각자 역할이 있다. `enable_security_metrics=True`는 5개 보안 트래커(Gate E)를 프로젝트 전체에 상시 켜두는 스위치다 — CLAUDE.md에 문서화된 대로, 외부 RAG 소스를 다루는 Book-forge의 성격상 항상 켜둔다. `enable_hallucination_detection=False`는 반대로 모니터 레벨에서는 꺼둔 값인데, `HallucinationDetector`는 이 전역 스위치가 아니라 에이전트별 `rag_mode=True`(7장 §7.4)로 개별적으로 켜지기 때문이다 — 모니터 레벨과 에이전트 레벨 중 어느 쪽이 이 기능의 온/오프를 결정하는지가 헷갈리기 쉬운 지점이다. `judge_sample_rate=0.2`는 `enable_llm_judge`가 켜졌을 때 전체 태스크 중 20%에만 LLM Judge를 적용한다는 뜻이다(전수 적용은 비용·지연이 크므로 표본만 채점). `auto_save=True`/`auto_save_interval=5`는 태스크 5개가 쌓일 때마다 `eval_results/*.json`에 자동으로 저장한다는 뜻으로, 세션 도중 프로세스가 죽어도 그 시점까지의 계측 결과가 파일로 남아 있게 한다.

## 16.4 `use_korean_tokenizer` — Book-forge 산출물의 언어에 맞춰 SDK를 조정한다
...
이 플래그를 켜는 것이 안전한 이유는 SDK 자신의 폴백 설계에 있다. `kiwipiepy`가 설치돼 있지 않으면 예외를 던지는 대신 **공백 분리로 자동 폴백**하고 경고 로그만 남긴다(이 챕터가 반복해온 "관대한 파싱" 원칙과 정확히 같은 자리에 있다). 그래서 이 값은 무조건 `True`로 요청해도 안전하다. `kiwipiepy`가 있으면 더 정확해지고, 없으면 기존과 동일하게 동작할 뿐이다. Book-forge는 `pyproject.toml`에 `korean = ["agent-evaluator[korean]"]`라는 옵트인 extra를 추가해, 이 정확도 향상을 원하는 사용자가 `pip install -e ".[korean]"`으로 선택할 수 있게 해뒀다.

> 👨‍💻 **개발자 TIP**: 이 발견은 "Agent-Evaluator SDK가 이미 제공하는 기능인데 Book-forge가 그동안 켜지 않고 있었다"는 점에서 12장(§12.2)의 `PerformanceMonitor.merge()`/`load_from_file()` 사례, 15장(§15.4)의 `.aoo/claims.jsonl` 사례와 같은 계보다. Book-forge와 Agent-Evaluator를 함께 쓸 때는 "새 기능을 요청하기 전에, 이미 있는 옵트인 파라미터부터 켜봤는가"를 먼저 점검할 가치가 있다.

## 16.5 관대한 파싱 — 오타 하나가 파이프라인을 죽이면 안 된다
...
## 16.6 실측 — `.env` 값이 실제로 반영되는지 API 키 없이 확인 가능하다

Gate 가중치 조정은 API 키가 필요 없는 경로다. `.env` 파일에 `BOOK_FORGE_GATE_A_TCR_WEIGHT=0.9`를 쓰면, 실제 `load_config()`(python-dotenv) → `os.environ` → `PerformanceMonitor(gate_a_tcr_weight=0.9)` 전체 경로가 몽키패치 없이 실측으로 확인됐다. `.env`에 쓴 값이 실제로 `PerformanceMonitor` 인스턴스의 내부 속성에 그대로 반영됨을 직접 실행으로 검증할 수 있었다.

---

## 직접 해보기

프로젝트 디렉토리(또는 저장소 루트)의 `.env`에 `BOOK_FORGE_GATE_A_TCR_WEIGHT=0.9`를 써넣고 `book-forge gate`를 다시 실행해보라. 같은 태스크 데이터인데 Gate A 점수가 달라지는 것을 직접 확인할 수 있다(§16.5). 값을 `zero point four`처럼 일부러 잘못 써넣어도 전체 파이프라인이 죽지 않고 그 항목만 조용히 기본값으로 돌아간다는 것(§16.4 관대한 파싱)도 함께 확인해보라.
