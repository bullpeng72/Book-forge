...
> **이런 분이 먼저 읽으면 좋습니다**: 지금까지 14개 챕터가 Book-forge를 잘 작동하는 사례로만 그렸는데, 실제로는 무엇이 부족한지 정직한 평가가 필요한 분.
...
> 이 책은 여기서 끝나지만, Book-forge와 Agent-Evaluator는 계속 바뀐다 — `00_기획안.md`에서 이미 밝혔듯 이 책의 근거는 특정 커밋 시점의 소스 코드다. 코드가 바뀌면 이 책의 서술도 낡아진다 — 그것이 "소스에 근거한 책"의 당연한 대가다. 최신 상태를 알고 싶다면, 언제나 이 책이 인용한 실제 파일로 돌아가라.
