# Chapter 15. 팀 동시성과 편집 충돌 방지

> **이 챕터에서 배우는 것** (이런 분이 먼저 읽으면 좋다: 14장의 `LiveGuardrail`이 "반복"을 막는 걸 봤는데, 같은 도구가 "동시 편집 충돌"이라는 완전히 다른 문제도 막을 수 있는지 궁금한 분)
...
## 15.2 저장 경로에 가드레일을 끼운다

14장에서 본 `@tool_guard` + `live_guardrail_session` 조합이 여기서도 그대로 재사용된다. 다만 이번엔 `LoopDetectionConfig`가 아니라 `TeamConcurrencyConfig`가 핵심이다.

> 📄 **파일**: `src/book_forge/editor/server.py`
...
## 15.4 새 클레임 로직을 만들지 않는다

`.aoo/claims.jsonl`이라는 파일 이름과 형식은 Book-forge가 새로 발명한 게 아니다. Agent-Evaluator SDK가 이미 제공하는 관례(`agent-eval claims add/list/release/audit`)를 프로젝트 디렉토리 안(`<project>/.aoo/claims.jsonl`)에 그대로 재사용한 것이다. 이 챕터 전체를 관통하는 원칙이 여기서도 반복된다(12장의 `merge()`/`load_from_file()` 재사용과 같은 정신). **새 판정 로직을 만들지 않고, 이미 검증된 SDK 기능을 정확한 지점에 연결하는 것.**
