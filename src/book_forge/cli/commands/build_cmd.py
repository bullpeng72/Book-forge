...
def build_pdf_cmd(slug: str, chapter: Optional[int]) -> None:
    """SLUG 프로젝트의 챕터를 Playwright로 PDF 인쇄한다."""
    config = load_book_config(slug)
    click.echo("📄 PDF 빌드 중 (Playwright)...")
    paths = _build_pdf(config, chapter_no=chapter)
    for p in paths:
